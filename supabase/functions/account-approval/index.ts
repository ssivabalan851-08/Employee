...
async function handleDecision(token: string, decision: string) {
  ...
  const rows = await json(await fetch(`${SUPABASE_URL}/rest/v1/account_approval_requests?approval_token_hash=eq.${tokenHash}&select=user_id,decision,decision_email_sent_at,profiles!inner(email,name,requested_role,approval_status)`, { headers: serviceHeaders }));
  ...
  if (!request.decision_email_sent_at) {
    const roleLabel = requestedRole === "manager" ? "HR & Admin Portal" : "Employee Portal";
    const content = approved
      ? `<h1 style="margin:0 0 14px;color:#063b39">Your LeaveWise account is approved</h1><p style="line-height:1.7">Hello <strong>${escapeHtml(profile.name)}</strong>,</p><p style="line-height:1.7;color:#365b58">Your request to access the <strong>${roleLabel}</strong> has been approved. You can now sign in using <strong>${escapeHtml(profile.email)}</strong>.</p><p><a href="${SITE_URL}" style="display:inline-block;background:#087d78;color:#fff;padding:13px 20px;border-radius:9px;text-decoration:none;font-weight:700">Sign in to LeaveWise</a></p>`
      : `<h1 style="margin:0 0 14px;color:#063b39">Update on your LeaveWise account request</h1><p style="line-height:1.7">Hello <strong>${escapeHtml(profile.name)}</strong>,</p><p style="line-height:1.7;color:#365b58">Your request to access the <strong>${roleLabel}</strong> was not approved at this time. If you believe this decision needs review, use the contact details below.</p>`;
    await sendEmail(profile.email, approved ? "Your LeaveWise account has been approved" : "Your LeaveWise account request decision", shell(content));
    await json(await fetch(`${SUPABASE_URL}/rest/v1/account_approval_requests?user_id=eq.${request.user_id}`, {
      method: "PATCH", headers: { ...serviceHeaders, Prefer: "return=representation" },
      body: JSON.stringify({ decision_email_sent_at: new Date().toISOString() }),
    }));
  }
  return resultPage(approved ? "Account approved" : "Account rejected", `The ${requestedRole === "manager" ? "HR" : "employee"} account decision was saved and ${profile.email} was notified.`);
}
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const rows = await json(await fetch(`${SUPABASE_URL}/rest/v1/account_approval_requests?user_id=eq.${userId}&select=email_sent_at,decision,profiles!inner(email,name,requested_role,department,title,approval_status)`, { headers: serviceHeaders }));
    ...
    const adminContent = `<h1 style="margin:0 0 14px;color:#063b39">New account approval request</h1><p style="line-height:1.7;color:#365b58">A new <strong>${escapeHtml(requestedLabel)}</strong> account is waiting for your decision.</p><table style="border-collapse:collapse;width:100%;margin:22px 0"><tr><td style="padding:10px;border-bottom:1px solid #d8ebe8;color:#67827f">Name</td><td style="padding:10px;border-bottom:1px solid #d8ebe8;font-weight:700">${escapeHtml(profile.name)}</td></tr><tr><td style="padding:10px;border-bottom:1px solid #d8ebe8;color:#67827f">Email</td><td style="padding:10px;border-bottom:1px solid #d8ebe8;font-weight:700">${escapeHtml(profile.email)}</td></tr><tr><td style="padding:10px;border-bottom:1px solid #d8ebe8;color:#67827f">Access</td><td style="padding:10px;border-bottom:1px solid #d8ebe8;font-weight:700">${escapeHtml(requestedLabel)}</td></tr><tr><td style="padding:10px;border-bottom:1px solid #d8ebe8;color:#67827f">Department</td><td style="padding:10px;border-bottom:1px solid #d8ebe8;font-weight:700">${escapeHtml(profile.department)}</td></tr><tr><td style="padding:10px;border-bottom:1px solid #d8ebe8;color:#67827f">Job title</td><td style="padding:10px;border-bottom:1px solid #d8ebe8;font-weight:700">${escapeHtml(profile.title)}</td></tr></table><p><a href="${functionUrl}?token=${token}&decision=approve" style="display:inline-block;background:#087d78;color:#fff;padding:13px 20px;border-radius:9px;text-decoration:none;font-weight:700;margin-right:8px">Review & approve</a><a href="${functionUrl}?token=${token}&decision=reject" style="display:inline-block;background:#be123c;color:#fff;padding:13px 20px;border-radius:9px;text-decoration:none;font-weight:700">Review & reject</a></p><p style="color:#67827f;font-size:12px;line-height:1.6">Each link opens a confirmation page. No decision is made until you confirm it.</p>`;
